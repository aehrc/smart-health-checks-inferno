...
module SmartHealthChecksTestKit
  module SmartHealthChecksV030_DRAFT
    class AllergyIntoleranceFHIRPathXMLPatchTest < Inferno::Test
      include InfernoSuiteGenerator::PatchTest

      title '(SHALL) Server returns correct AllergyIntolerance resource from AllergyIntolerance patch interaction (FHIRPath Patch in XML format)'
      description 'A server SHALL support the AllergyIntolerance patch interaction (FHIRPath Patch in XML format).'

      id :smart_health_checks_v030_draft_allergy_intolerance_fhirpath_xml_patch_test
