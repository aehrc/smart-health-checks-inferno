...
class AidboxConfigurer
  ...
  def add_step(path, method, body, headers)
    @steps << AidboxConfigurerStep.new(@base_url, path, method, body, headers)
  end
  ...
  def add_upload_step(path, file_path, headers = {}, form_field: 'file')
    ...
    @steps << AidboxConfigurerStep.new(@base_url, path, 'POST', body, headers)
  end
...
configurer = AidboxConfigurer.new(base_url)
